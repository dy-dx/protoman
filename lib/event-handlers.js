...
module.exports = {
  player_death: function (event) {
    ...
    if (event.data.death_flags === 160) {
      ...
      return victim.addValue('feignedDeaths', 1);
    }
    ...
    if (event.data.attacker === event.data.userid) {
      // Suicides
      this.getPlayer(event.data.userid).addValue('suicides', 1);
...
, player_invulned: function (event) {
    ...
    this.getPlayer(event.data.userid).addValue('ubered', 1);
  }
...
, player_builtobject: function (event) {
    ...
    if (event.data.object === 2) {
      player.addValue('sentriesBuilt', 1);
    }
}
